...
class EncoderAnySplat(Encoder[EncoderAnySplatCfg]):
    ...
    def forward(
        ...
        # dpt style gs_head input format   pts_all [B,V,H,W,3]
        B_tensor = torch.zeros(b,v,h,w,dtype=torch.uint8)
        for bval in range(b):
            B_tensor[bval,:,:,:] = bval
        B_tensor = B_tensor.flatten(2,3)
        V_tensor = torch.zeros(b,v,h,w,dtype=torch.uint8)
        for vval in range(v):
            V_tensor[:,vval,:,:] = vval
        V_tensor = V_tensor.flatten(2,3)
        ...
        pts_flat = pts_all.flatten(2, 3)  # pts_flat [B,V,H*W,3]
